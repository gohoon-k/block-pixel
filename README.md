...
거의 모든 버전관리와 관련된 명령은 __dimension__ 인수를 받는데, 이것은 실제 마인크래프트 클라이언트가 읽는 디렉터리로부터 버전관리가 진행되는 로컬 저장소 디렉터리로 복사하여 
로컬 저장소의 변경사항을 만들 차원을 지정하거나, 로컬 저장소로부터 마인크래프트 클라이언트가 읽는 디렉터리로의 복사를 통해 실제 맵에 반영할 차원을 지정하는 인수입니다.  
따라서 이 __dimension__ 인수에 포함되지 않는 차원은 실제 맵에도, 로컬저장소에도 <b>`/pixel` 커맨드 작업</b>에 따른 변경사항이 반영되지 않습니다.  
  
예를 들어, 마인크래프트를 통해 오버월드, 네더에 변경사항을 만들고 overworld 라는 dimension 인수를 통해 commit 을 수행할 경우 해당 커밋에는 overworld 의 변경사항만 포함됩니다.
  
나머지 사항은 Git 의 그것과 최대한 비슷하도록 구성했습니다.  
  
몇몇 커맨드를 수행하면 엔터를 치자 마자 갑자기 월드가 사라지는 것처럼 보이는데, 월드를 언로드하여 데이터를 읽기 위해 잠시 해당 차원에 있는 플레이어를 공허 세계<sup>[[4]](#메인-월드와-버전관리-차원)</sup>로 이동시킨 것입니다.  
...
### 메인 월드와 버전관리 차원
플러그인이 처음 활성화될 때, 두 개의 월드 <b>void</b>(공허세계), <b><i>level_name</i>_overworld</b>(생성 시점의 메인 월드의 복제)를 새로 생성합니다.
그러면 일반적인 경우 서버에는 총 다섯 개의 월드가 존재하게 됩니다.  
그 중 <b><i>level_name</i>_overworld</b>, <b><i>level_name</i>_nether</b>, <b><i>level_name</i>_the_end</b>는 버전관리 차원, <b><i>level_name</i></b>은 메인 월드가 됩니다.

그러나 메인 월드와 버전관리가 진행되는 월드의 베이스는 같기 때문에, 간혹 어느 월드에 있는지 헷갈릴 때가 있을 수 있습니다.  
그럴 때 `/pixel whereis` 커맨드를 통해 현재 월드를 파악하거나, `/pixel tp` 커맨드를 사용하여 가려는 월드로 텔레포트할 수 있습니다.


## 사용 가능한 커맨드
- ```/pixel init```  
Git 로컬 저장소를 초기화합니다. 이미 저장소가 있을 경우 <b>기존 것은 삭제되고 다시 초기화</b><sup>[[1]](#저장소-초기화)</sup>됩니다.
  

- ```/pixel commit <dimension> <commit_message>```  
Git 커밋을 진행합니다.
  - dimension : 변경사항을 커밋에 포함할 차원
  - commit_message : 커밋 메시지. 띄어쓰기를 해도 된다.
  

- ```/pixel reset <dimension> <steps|commit_hash>```  
Git 리셋을 진행합니다. 헤드의 위치를 n커밋 뒤로 옮기거나 특정 커밋으로 옮깁니다.
  - dimension : 리셋을 반영할 차원
...
플레이어를 지정한 월드의 현재 차원과 동일한 좌표 위치로 텔레포트합니다.<sup>[[4]](#메인-월드와-버전관리-차원)</sup>
  - target : 텔레포트할 플레이어의 닉네임
  - dimension : 도착 월드. dummy 혹은 overworld 중 하나.
  

- ```/pixel whereis <target>```  
플레이어가 현재 어느 차원에 있는지를 출력합니다.
